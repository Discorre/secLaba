...
int main() {
    MySet<int> mySet; // Создаем множество для целых чисел

    // Добавление элементов
    mySet.add(1);
    mySet.add(2);
    mySet.add(3);
    mySet.print(); // Вывод: Множество: { 1 2 3 }

    // Проверка наличия элементов
    std::cout << "Содержит 2? " << (mySet.contains(2) ? "Да" : "Нет") << std::endl; // Да
    std::cout << "Содержит 4? " << (mySet.contains(4) ? "Да" : "Нет") << std::endl; // Нет

    // Удаление элемента
    mySet.remove(2);
    mySet.print(); // Вывод: Множество: { 1 3 }

    // Попытка удаления несуществующего элемента
    mySet.remove(4); // Элемент 4 не найден в множестве.
